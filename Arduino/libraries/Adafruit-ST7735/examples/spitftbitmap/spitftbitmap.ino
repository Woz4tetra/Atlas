...
#define LITE     3  // Backlight control
...
int brightness = 255;
long long time_start = millis();
char *file_name = "img.bmp";
...
void setup(void) {
  ...
  pinMode(LITE, OUTPUT);
  analogWrite(LITE, brightness % 256);
...
void loop()
{
    if (millis() - time_start > 2000)
    {
        ...
        for(uint8_t i=0; i<4; i++)    // Draw 4 parrots
            bmpDraw(file_name, tft.width() / 4 * i, tft.height() / 4 * i);
        delay(1000);
