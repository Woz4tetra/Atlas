...
void BasicStepperDriver::setGoal(long goal) {
    if (current_step == goal) {
        return;
    }
    if ((goal - current_step) >= 0){
        setDirection(1);
    } else {
        setDirection(-1);
    }
...
void BasicStepperDriver::run() {
    if (current_step != goal_step && (micros() - last_step_time) > pulse_duration)
    {
        digitalWrite(step_pin, pinState ? HIGH : LOW);
        pinState = !pinState;
...
long BasicStepperDriver::getGoalStep() {
    return goal_step;
}

long BasicStepperDriver::getCurrentStep() {
    return current_step;
}

long BasicStepperDriver::distToGoal() {
    return goal_step - current_step;
}
