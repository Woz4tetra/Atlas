...
void setup()
{
    ...
    // No initialization data. receive_first in adafruit_gps.py will not be called
}
...
void loop()  // run over and over again
{
    while (buggy.available()) {
        ...
        if (status == 2) {  // start event
            ...
            // Example of sensor that requires a different baud. Hardware and software serial
            // require the baud rates to be the same for some reason...
            // In adafruit_gps.py, self.baud is set to 9600 to match
            buggy.changeBaud(GPS_BAUD_RATE);
        }
        else if (status == 1) {  // stop event
            GPS.standby();
            useInterrupt(false);
            buggy.changeBaud(DEFAULT_RATE);  // put baud rate back on exit
        }
        else if (status == 5) {
            useInterrupt(false);
        }
