...
class INS:
    ...
    def get_earth_rotation_matrix(self, dt: float): #todo is this update rate the same or faster than the call rate? dt i mean
    ...
    def update(self, dt, accel_measurement: np.matrix,
        ...
        estimated_attitude = \
            self.get_estimated_attitude(
                ...
                self.properties.estimated_attitude  # todo check update rate of this shit
            )
        ...
        estimated_velocity = \
            prev_est_v + dt * (
                accel_meas_ecef - 2 * skew_symmetric(  #todo check if shapes right here?? and the 2 thing
...
class Epoch:
    ...
    def update(self, dt: float, gps_position_ecef: np.matrix,
        ...
        self.determine_transition_matrix(dt, accel_measurement)  # step 1 #todo hmnnn is no returns intended?
    ...
    def determine_transition_matrix(self, dt, accel_measurement):
        ...
        self.state_transition_Phi[0:3, 0:3] -= self.skew_earth_rotation * dt #todo this seems wierd. is the minus a typo?
        ...
        self.state_transition_Phi[3:6, 3:6] -= 2 * self.skew_earth_rotation * dt #todo same as above. maybe cap these?
...
def gravity_ecef(ecef_vector):
    ...
    # Calculates gravitational acceleration in ECEF
        ...
        z_scale = unit_to_scalar((ecef_vector[2] / mag_r) ** 2 * 5) # todo clear this error ben, idk what it wants
        ...
        gamma = np.matrix(-earth_gravity_constant / mag_r ** 3 *  # todo is this an array of one or are we missing comma
                          (ecef_vector + 1.5 * J_2 * (
