...
class RDataGenerator(Simulator):
    def __init__(self, file_name, directory, map_name, map_dir, FilterClass,
        ...
        super(RDataGenerator, self).__init__(
            file_name, directory, filter, plot_info
        )

        r_data_dir = os.path.join(
            project.get_dir(":r_data"),
            "Filter Data " + self.parser.file_name_no_ext + ".csv")

        print("Writing R data to")
        print(r_data_dir)
        self.r_data = open(r_data_dir, 'w', newline='')
        self.r_data_csv = csv.writer(self.r_data)

        self.r_data_csv.writerow([
            "state x", "state y",
            "gps x", "gps y",
    ...
    def step(self, index, timestamp, name, values):
        ...
        if name == "imu":
            ...
            gps_x, gps_y = values["long"], values["lat"]
            sensor_updated = True
            ...
            checkpoint_x, checkpoint_y = self.checkpoints[values["num"]]
            sensor_updated = True

        if sensor_updated:
            self.r_data_csv.writerow(
                [self.filter.state["x"], self.filter.state["y"],
                 gps_x, gps_y, checkpoint_x, checkpoint_y]
            )
