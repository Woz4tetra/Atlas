...
class LidarTurret(RobotObject):
    def __init__(self, ticks_per_rotation=38):
        ...
        self.distances = [0] * ticks_per_rotation
        self.point_cloud = [(0, 0)] * ticks_per_rotation
        self.point_cloud_x = [0] * ticks_per_rotation
        self.point_cloud_y = [0] * ticks_per_rotation

        super(LidarTurret, self).__init__("lidar", "/dev/tty.usbmodem*")
...
class PyAccel(RobotObject):
    ...
    def set_led(self, *led_states):
        self.write_packet(("%i\t" * 4) % led_states)
