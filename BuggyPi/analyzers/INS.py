...
'''
    def __init__(self, t):
        ...
        self.time = t # this will be used to calculate the time needed for dt
    ...
    def update(self, ax, ay, yaw, t):
        ...
        if length >= self.max: # only need to do more if there are 10 already
            ...
            #used state ax and state ay and state w for consistency and bc it makes sense
            
            dt = t - self.time # calculate time during which the imu entries were averaged
            #now update the thing
            self.state.ax   = float((avg_ax + self.state.ax) / 2.0)
            # averaging these so the past is used in some fashion (prevents explosions a bit)
            ...
            self.temp = [] # reinistialize temp to be empty
            self.time = t # reinisialze time

    def print_state(self):
        print("ax = %2.2f, ay = %2.2f, vx = %2.2f, vy = %2.2f, x = %2.2f, y = %2.2f, yaw = %2.2f" % (
            self.state.ax, self.state.ay, self.state.vx, self.state.vy, self.state.x,
            self.state.y, self.state.yaw))
...
#change to make it where it uses an inputted t, ignoring it unless its used (every 10)
#figure out how to reconcile the starting position with the absolute position of ned (north east down) RESEARCH
