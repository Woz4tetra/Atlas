...
def main():
    ...
    joystick.start()
    start(log_data=True, soft_reboot=False)
...
##            else:
##                leds[3].set(0)
            print("%0.4f, %5.0i, (%0.6f, %0.6f, %i), motors: %3.0i, servo: %3.0i" % (imu.get('yaw'), encoder.get('counts'),
                  gps.get('lat'), gps.get('long'), gps.get('found'), motors.get(), servo.get()), end='\r')
