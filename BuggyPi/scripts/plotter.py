...
class FilterPlotter:
    def __init__(self, file_name, directory=None):
        ...
        self.pi_filter = BuggyPiFilter(
            self.initial_long, self.initial_lat, 0.0, 6, 0.097, 0.234,
            0.8869709636028431
        )
    ...
    def update_state_data(self, state):
        ...
        if self.heading_counter % 50 == 0:
            x0 = self.state_x[-1]
            y0 = self.state_y[-1]
            percent_vx = (self.pi_filter.max_speed - state[
                "vx"]) / self.pi_filter.max_speed
            percent_vy = (self.pi_filter.max_speed - state[
                "vy"]) / self.pi_filter.max_speed
            x1 = x0 + 0.00001 * percent_vx * math.cos(state["angle"])
            y1 = y0 + 0.00001 * percent_vy * math.sin(state["angle"])
    ...
    def create_map(self, skip):
        ...
        map_file.write("num, long, lat\n")
        for index in range(0, len(self.state_x), skip):
            map_file.write("%s, %s, %s\n" % (
                index, self.state_x[index], self.state_y[index]))
